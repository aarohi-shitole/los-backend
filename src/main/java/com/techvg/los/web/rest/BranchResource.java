...
import com.techvg.los.service.BranchService;
...
/**
 * REST controller for managing {@link com.techvg.los.domain.Branch}.
 */
@RestController
@RequestMapping("/api")
public class BranchResource {
     ...
     *         body the new branchDTO, or with status {@code 400 (Bad Request)} if
     *         the branch has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/branches")
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN','BRANCH_EDIT')")
     ...
     *         the updated branchDTO, or with status {@code 400 (Bad Request)} if
     *         the branchDTO is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the branchDTO couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    /**
     * {@code PATCH  /branches/:id} : Partial updates given fields of an existing
     * branch, field will ignore if it is null
     *
     * @param id        the id of the branchDTO to save.
     * @param branchDTO the branchDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated branchDTO, or with status {@code 400 (Bad Request)} if
     *         the branchDTO is not valid, or with status {@code 404 (Not Found)} if
     *         the branchDTO is not found, or with status
     *         {@code 500 (Internal Server Error)} if the branchDTO couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
     ...
     */
    @GetMapping("/branches")
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN','BRANCH_VIEW','DASHBOARD_CEO_VIEW','BRANCH_LIST_VIEW')")
    ...
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN','BRANCH_VIEW','BRANCH_LIST_VIEW')")
    public ResponseEntity<Count> countBranches(BranchCriteria criteria) {
        log.debug("REST request to count Branches by criteria: {}", criteria);
        Count branchesCount = new Count();
        branchesCount.count = branchQueryService.countByCriteria(criteria);
        return ResponseEntity.ok().body(branchesCount);
    }
    ...
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'BRANCH_VIEW','BRANCH_EDIT')")
