...
import com.techvg.los.service.MemberBankService;
...
/**
 * REST controller for managing {@link com.techvg.los.domain.MemberBank}.
 */
@RestController
@RequestMapping("/api")
public class MemberBankResource {
     ...
     *         body the new memberBankDTO, or with status {@code 400 (Bad Request)}
     *         if the memberBank has already an ID.
    ...
    public ResponseEntity<MemberBankDTO> createMemberBank(@Valid @RequestBody MemberBankDTO memberBankDTO) throws URISyntaxException {
        ...
        Optional<MemberBankDTO> memberBank = memberBankService.findAccountNumber(memberBankDTO.getAccountNumber());

        if (memberBank.isPresent()) {
            throw new BadRequestAlertException("MemberBank already exist for give Account Number", ENTITY_NAME, "idexists");
        }
     ...
     *         the updated memberBankDTO,
     *         or with status {@code 400 (Bad Request)} if the memberBankDTO is not
     *         valid,
     *         or with status {@code 500 (Internal Server Error)} if the
     *         memberBankDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    /**
     * {@code PATCH  /member-banks/:id} : Partial updates given fields of an
     * existing memberBank, field will ignore if it is null
     *
     * @param id            the id of the memberBankDTO to save.
     * @param memberBankDTO the memberBankDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     ...
     *         or with status {@code 400 (Bad Request)} if the memberBankDTO is not
     *         valid,
     *         or with status {@code 404 (Not Found)} if the memberBankDTO is not
     *         found,
     *         or with status {@code 500 (Internal Server Error)} if the
     *         memberBankDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
    ...
    public ResponseEntity<List<MemberBankDTO>> getAllMemberBanks(
        ...
        log.debug("REST request to get MemberBanks by criteria: {}", criteria);
    ...
    public ResponseEntity<Long> countMemberBanks(MemberBankCriteria criteria) {
        log.debug("REST request to count MemberBanks by criteria: {}", criteria);
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the memberBankDTO, or with status {@code 404 (Not Found)}.
     */
