...
import com.techvg.los.service.TalukaQueryService;
import com.techvg.los.service.TalukaService;
...
/**
 * REST controller for managing {@link com.techvg.los.domain.Taluka}.
 */
@RestController
@RequestMapping("/api")
public class TalukaResource {
     ...
     *         body the new talukaDTO, or with status {@code 400 (Bad Request)} if
     *         the taluka has already an ID.
     ...
     *         the updated talukaDTO, or with status {@code 400 (Bad Request)} if
     *         the talukaDTO is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the talukaDTO couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public ResponseEntity<TalukaDTO> updateTaluka(
        ...
        TalukaDTO result = talukaService.update(talukaDTO);
        return ResponseEntity
            .ok()
    ...
    /**
     * {@code PATCH  /talukas/:id} : Partial updates given fields of an existing
     * taluka, field will ignore if it is null
     *
     * @param id        the id of the talukaDTO to save.
     * @param talukaDTO the talukaDTO to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated talukaDTO, or with status {@code 400 (Bad Request)} if
     *         the talukaDTO is not valid, or with status {@code 404 (Not Found)} if
     *         the talukaDTO is not found, or with status
     *         {@code 500 (Internal Server Error)} if the talukaDTO couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the talukaDTO, or with status {@code 404 (Not Found)}.
     */
