...
import { EntityArrayResponseType, SecurityPermissionService } from '../service/security-permission.service';
...
export class SecurityPermissionComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISecurityPermission): number => this.securityPermissionService.getSecurityPermissionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.securityPermissions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISecurityPermission[] | null): ISecurityPermission[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.securityPermissionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
