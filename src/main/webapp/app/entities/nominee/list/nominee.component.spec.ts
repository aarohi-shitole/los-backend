...
describe('Nominee Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'nominee', component: NomineeComponent }]), HttpClientTestingModule],
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.nominees?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to nomineeService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getNomineeIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getNomineeIdentifier).toHaveBeenCalledWith(entity);
