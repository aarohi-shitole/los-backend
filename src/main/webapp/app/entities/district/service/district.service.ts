...
type RestOf<T extends IDistrict | NewDistrict> = Omit<T, 'lastModified'> & {
...
export class DistrictService {
  ...
  protected convertDateFromClient<T extends IDistrict | NewDistrict | PartialUpdateDistrict>(district: T): RestOf<T> {
    return {
      ...district,
      lastModified: district.lastModified?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDistrict: RestDistrict): IDistrict {
    return {
      ...restDistrict,
      lastModified: restDistrict.lastModified ? dayjs(restDistrict.lastModified) : undefined,
    };
  }
