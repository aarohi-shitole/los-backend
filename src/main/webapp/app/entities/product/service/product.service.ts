...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'lastModified'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      lastModified: product.lastModified?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      lastModified: restProduct.lastModified ? dayjs(restProduct.lastModified) : undefined,
    };
  }
