...
type RestOf<T extends ISecurityUser | NewSecurityUser> = Omit<T, 'resetDate' | 'createdOn' | 'lastModified'> & {
...
export class SecurityUserService {
  ...
  protected convertDateFromClient<T extends ISecurityUser | NewSecurityUser | PartialUpdateSecurityUser>(securityUser: T): RestOf<T> {
    return {
      ...
      lastModified: securityUser.lastModified?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSecurityUser: RestSecurityUser): ISecurityUser {
    return {
      ...
      lastModified: restSecurityUser.lastModified ? dayjs(restSecurityUser.lastModified) : undefined,
    };
  }
